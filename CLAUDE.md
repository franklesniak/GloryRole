...
This repository uses modular instruction files for language-specific coding standards. **Read the relevant instruction file before modifying files that match its pattern unless it is already present in your current agent context; if it is already present, you may rely on the injected copy and only re-read from disk when you need to verify the repository version.**
